...
class CocotbBaseTestCase():
    ...
    def simulate(
        ...
        self.cocotb_simulate(top.__class__.__name__,io, tv)

    # tv-> 提取测试向量中的 IOStruct
    ...
    # 生成 cocotb 测试文件 & Makefile
    def cocotb_simulate(
        self, top_module_name:str,io:HDL.IOStruct, tv:list
    ):
        ...
        # 生成 cocotb 测试  以及是否使用runner
        self._generate_cocotb_test(top_module_name, ports, vectors,use_runner=True)
    ...
    def _generate_cocotb_test(self, top: str, ports: list, vectors: list, use_runner: bool = True) -> None:
        ...
        if use_runner:
            lines.append("from cocotb_tools.runner import get_runner")
        ...
        lines.append("    await Timer(1, units='step')")
        lines.append("")
        ...
        lines.append("")
        # 根据开关决定是否注入 runner 代码
        if use_runner:
            lines.append(self._generate_runner_code(top))
            print("Gnerating cocotb runner code.")
    ...
    def _generate_runner_code(self, top: str) -> str:
        """生成基于 cocotb-tools runner 的 Python 代码块"""
        return textwrap.dedent(f"""
            def test_{top}_runner():
                ...
                if hdl_toplevel_lang == "verilog":
                    sources = [proj_path / "{top}.sv"]
                else:
                    sources = [proj_path / "{top}.vhdl"]
